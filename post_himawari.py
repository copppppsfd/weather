...
def write_last_slot(slot_label: str):
...
def main():
    ...
    last_slot = read_last_slot()
    if slot_label == last_slot:
        print(f"Slot {slot_label} already posted, skipping.")
        return

    post_to_discord(image_bytes, slot_label)
    write_last_slot(slot_label)
    print(f"Posted image for slot {slot_label}")
